...
class BatchedEngine(BaseEngine):
    ...
    async def _start_llm(self) -> None:
        ...
        # ``dispatch_mtp_inject`` runs BELOW and attaches ``model.mtp`` at
        # that point — so a pre-dispatch legacy validate is guaranteed to
        ...
        # the new-arch dispatcher will run instead (root-cause investigation
        # Phase 1 cleanup).
        sc = self._scheduler_config
