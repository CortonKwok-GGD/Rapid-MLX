...
def _install_mtp_vendored(
    ...
    * On the very first ``_step`` call we short-circuit and return the
    ...
    def _is_greedy_for_uid(uid: int) -> bool:
        ...
        MVP: matches the greedy contract that
    ...
    logger.info(
        ...
        "(single-request greedy MVP; falls through on B>1 / non-greedy / "
        "logits-processors)."
    )
    return True
...
class Scheduler:
    ...
    def _create_batch_generator(
        ...
        # 0.9.11 PR-4 (MVP): server-side wiring for ``--spec-decode mtp``.
        # This installs the vendored PR #990 ``mtp_generate_step`` hot
        # loop as ``GenerationBatch._step``, gated on target having the
        # ``mtp_forward`` / ``make_mtp_cache`` protocol installed by
        # ``dispatch_mtp_inject`` (which ran during engine boot in
        # ``BatchedEngine._start_llm`` before this scheduler was built).
        #
        ...
        # (as the Gemma 4 MVP smoke does today: ``--enable-mtp
        # --spec-decode mtp``), the legacy install logs "no _step" and
        # returns False — this install is what actually runs the MTP
        # draft/verify loop.
