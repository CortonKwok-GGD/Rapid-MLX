...
struct ToolUseCapabilityCatalogCoverageTests {
    ...
    /// Walk the live ``aliases.json`` (decoded at test-time from the
    /// submodule snapshot the build embeds) and assert every alias
    /// resolves to its expected bucket. New aliases that show up in
    /// the catalog without an entry in ``expectedMatrix`` fail the
    /// test with a "please add a row" message so the reviewer makes
    /// a deliberate call instead of letting a silent default land.
    @Test("Every alias in aliases.json resolves to its expected bucket")
    func catalogWalkMatchesExpectedMatrix() throws {
        let aliases = try Self.loadCatalogAliases()
        try #require(!aliases.isEmpty, "Catalog walk found 0 aliases — submodule not initialised? Run `git submodule update --init third_party/rapid-mlx`.")

        // Coverage check: every aliases.json entry must have an
        // expectation row. Surface missing entries as a single
        // structured failure so the reviewer sees the new alias
        // names at a glance instead of one failure per missing row.
        let missingEntries = aliases.filter { Self.expectedMatrix[$0] == nil }
        #expect(
            missingEntries.isEmpty,
            "Catalog has aliases without an entry in expectedMatrix: \(missingEntries.sorted()). Add a deliberate classification (.known / .broken / .unknown) for each new alias and document the reasoning."
        )

        // Inverse coverage: every expectedMatrix entry must
        // correspond to a live catalog alias — otherwise the
        // expectation table has drifted (alias renamed / removed).
        let aliasSet = Set(aliases)
        let staleEntries = Self.expectedMatrix.keys.filter { !aliasSet.contains($0) }
        #expect(
            staleEntries.isEmpty,
            "expectedMatrix has aliases that no longer exist in the catalog: \(staleEntries.sorted()). Did the submodule bump rename/drop them?"
        )

        // Per-alias confidence check.
        for alias in aliases.sorted() {
            guard let expected = Self.expectedMatrix[alias] else { continue }
            let actual = ToolUseCapability.confidence(for: alias)
            #expect(
                actual == expected,
                "Alias '\(alias)' resolved to .\(actual) but matrix expected .\(expected). If the classification has legitimately changed, update expectedMatrix with the cycle/PR rationale."
            )
        }
    }

    /// Aggregate distribution guard — pin the count of each bucket
    /// so a future PR that silently widens ``.unknown`` (e.g. by
    /// dropping a family from the verified set) shows up as a
    /// count drift here.
    ///
    /// Pre-fix HEAD on this catalog: 21 known / 3 broken / 68 unknown.
    /// Post-fix expected: 54 known / 5 broken / 33 unknown.
    /// v0.8.11 submodule bump (PR #379): +11 ``.unknown`` (9 ui-tars +
    /// 2 embeddinggemma); 54 known / 5 broken / 44 unknown out of 103.
    /// v0.8.19 submodule bump (rapid-mlx 0.9.9): 64 known / 5 broken /
    /// 59 unknown out of 128.
    /// 2026-07-09 recommended-model tool-usability sweep: nine aliases
    /// moved to ``.broken`` — 3 from ``.unknown`` (phi-4-mini-4bit,
    /// deepseek-coder-v2-lite-16b-4bit, deepseek-r1-8b-4bit) and 6 from
    /// ``.known`` (the whole Mistral family: mistral-24b-4bit,
    /// mistral-small-4-119b ×3, devstral-24b-4bit, devstral-v2-24b-4bit
    /// — engine parser misconfig, re-promote when the "mistral" parser
    /// ships bundled).
    /// v0.10.5 submodule reconciliation (2026-07-09, #514): the catalog
    /// the test actually loads grew to 158 aliases (30 new gpt-oss /
    /// qwen3.6 / gemma-4 variants). Full live distribution is now
    /// 85 known / 14 broken / 59 unknown out of 158 (the 30 new rows add
    /// 27 ``.known`` size-siblings of verified families + 3 ``.unknown``
    /// sub-3B gemma-4-e2b variants). Counts below run over the FULL live
    /// catalog, not the classified subset.
    /// bonsai-starter bump (submodule 3050660 → 7b6a787): the Mistral /
    /// Devstral family (6 aliases) is RE-PROMOTED .broken → .known now
    /// that the engine ships the ``mistral`` parser (rapid-mlx
    /// #1071/#1077; devstral-v2-24b re-benched clean on the bundled
    /// engine), so broken 14 → 8 and those 6 join ``.known``. The same
    /// bump also drops 3 FP16 bonsai-unpacked (.unknown), adds
    /// bonsai-1.7b-2bit (.known), and adds LFM2.x ×3 + Hy3 ×1 (.unknown)
    /// → unknown nets +1 (59 → 60).
    @Test("Aggregate distribution: <=15 unknown is the goal; current shipping ceiling is 60")
    func aggregateDistributionMeetsFloor() throws {
        let aliases = try Self.loadCatalogAliases()
        try #require(!aliases.isEmpty)

        var counts: [ToolUseConfidence: Int] = [.known: 0, .broken: 0, .unknown: 0]
        for alias in aliases {
            counts[ToolUseCapability.confidence(for: alias), default: 0] += 1
        }

        let known = counts[.known] ?? 0
        let broken = counts[.broken] ?? 0
        let unknown = counts[.unknown] ?? 0

        // .known should dominate the post-fix catalog (pre-fix: 21,
        // post-fix: 54). Pin a floor so a future regression that
        // demotes a family surfaces here.
        #expect(
            known >= 50,
            "Catalog .known count dropped to \(known) (pre-fix-floor=21, post-fix-floor=50). A family was demoted or aliases dropped from the verified set without updating the floor."
        )

        // .broken should track the empirical denylist exactly.
        // Pre-fix: 3 (3 broken aliases × 1 catalog match each).
        // v0.8.x: 5 (added vibethinker x2).
        // 2026-07-09 sweep: 14. The +9 were phi-4-mini-4bit,
        // deepseek-coder-v2-lite-16b-4bit, deepseek-r1-8b-4bit (Cat A —
        // model can't tool-call) plus the 6-strong Mistral family
        // (Cat B, engine parser misconfig).
        // bonsai-starter bump (7b6a787): the 6 Mistral/Devstral aliases
        // are re-promoted to .known now that the "mistral" parser ships
        // bundled (rapid-mlx #1071/#1077), so 14 → 8. The remaining 8
        // are the two Cat-A groups above (Mistral was the only Cat-B).
        #expect(
            broken == 8,
            "Catalog .broken count drifted to \(broken); expected exactly 8 (phi-4-mini-reasoning, hermes3-8b, llama3-1b, vibethinker-1.5b, vibethinker-3b, phi-4-mini-4bit, deepseek-coder-v2-lite-16b-4bit, deepseek-r1-8b-4bit). A new .broken entry needs the matching test pin + bug_report citation; the Mistral/Devstral family was re-promoted to .known when the mistral parser shipped bundled."
        )

        // .unknown is the residual — the count should drop dramatically
        // from 68 to ~33 with the family-aware fix. If it climbs back
        // toward 68, the family classifier has broken. v0.8.11 bump
        // raised the post-fix floor by 11 (9 ui-tars + 2 embeddinggemma
        // legitimately ``.unknown`` per their families never being
        // benched by the loop), so the ceiling moved 35 → 46.
        // v0.8.19 bump (rapid-mlx v0.9.8 → v0.9.9) adds 14 legitimately
        // ``.unknown`` aliases — tmax (7), holo3.1 (2), glm-5.2-reap50,
        // kimi-k2.6, and sub-3B qwen3-0.6b/1.7b (3) — none benched by
        // the loop, so the ceiling moves 46 → 59.
        // bonsai-starter bump (rapid-mlx 3050660 → 7b6a787) nets +1:
        //   −3  FP16 ``bonsai-*-unpacked`` dropped (#1092, were .unknown)
        //   +0  ``bonsai-1.7b-2bit`` added but is .known
        //   +4  LFM2.x (#1076, ×3) + Hy3 preview (#1070, ×1) — engine
        //       parsers landed but the desktop loop hasn't benched them
        // so the ceiling moves 59 → 60.
        #expect(
            unknown <= 60,
            "Catalog .unknown count climbed to \(unknown) (pre-fix: 68, ceiling: 60 after the bonsai-starter bump). The family-aware classifier likely regressed — a missing family entry causes every sibling to silently fall back to .unknown."
        )
    }
