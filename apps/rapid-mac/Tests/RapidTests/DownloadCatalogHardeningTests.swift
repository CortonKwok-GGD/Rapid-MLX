...
struct DownloadCatalogHardeningTests {
    ...
    @Test("#298: hasAnyCachedHFRepo — nil root returns false")
    func hasAnyCachedHFRepoNilRoot() {
        #expect(ModelCatalog.hasAnyCachedHFRepo(hubCacheRoot: nil) == false)
    }

    @Test("#298: hasAnyCachedHFRepo — missing dir returns false")
    func hasAnyCachedHFRepoMissingDir() {
        let url = URL(fileURLWithPath: "/tmp/nonexistent-\(UUID().uuidString)", isDirectory: true)
        #expect(ModelCatalog.hasAnyCachedHFRepo(hubCacheRoot: url) == false)
    }

    @Test("#298: hasAnyCachedHFRepo — empty dir returns false (fresh install)")
    func hasAnyCachedHFRepoEmptyDir() throws {
        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("rapid-quickstart-cache-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tmp, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tmp) }
        #expect(ModelCatalog.hasAnyCachedHFRepo(hubCacheRoot: tmp) == false)
    }

    @Test("#298: hasAnyCachedHFRepo — only non-models-- entries returns false")
    func hasAnyCachedHFRepoOnlyNonModelsEntries() throws {
        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("rapid-quickstart-cache-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tmp, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tmp) }
        // .DS_Store + version file + a non-conforming subdir — none start with "models--"
        try Data().write(to: tmp.appendingPathComponent(".DS_Store"))
        try Data().write(to: tmp.appendingPathComponent("version.txt"))
        try FileManager.default.createDirectory(
            at: tmp.appendingPathComponent("datasets--foo--bar", isDirectory: true),
            withIntermediateDirectories: true
        )
        #expect(ModelCatalog.hasAnyCachedHFRepo(hubCacheRoot: tmp) == false)
    }

    @Test("#298: hasAnyCachedHFRepo — at least one models-- dir returns true (upgrade user)")
    func hasAnyCachedHFRepoHasModelDir() throws {
        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("rapid-quickstart-cache-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tmp, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tmp) }
        // Simulate one cached repo — exact layout HF uses.
        try FileManager.default.createDirectory(
            at: tmp.appendingPathComponent("models--mlx-community--Qwen3-0.6B-4bit", isDirectory: true),
            withIntermediateDirectories: true
        )
        #expect(ModelCatalog.hasAnyCachedHFRepo(hubCacheRoot: tmp) == true)
    }

    @Test("#298: hasAnyCachedHFRepo — symlink to models-- dir counts (bundled-snapshot install)")
    func hasAnyCachedHFRepoSymlinkCounts() throws {
        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("rapid-quickstart-cache-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tmp, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tmp) }
        // BundledModel.installBundledSnapshotSymlink links the bundled
        // snapshot directory in via a symlink — pin that shape so the
        // gate also fires for bundled-snapshot installs (the v0.7.1
        // airgapped build path).
        let target = tmp.appendingPathComponent("real-target", isDirectory: true)
        try FileManager.default.createDirectory(at: target, withIntermediateDirectories: true)
        let link = tmp.appendingPathComponent("models--mlx-community--Qwen3-0.6B-4bit")
        try FileManager.default.createSymbolicLink(at: link, withDestinationURL: target)
        #expect(ModelCatalog.hasAnyCachedHFRepo(hubCacheRoot: tmp) == true)
    }

    @Test("#298: hasAnyCachedHFRepo — broken symlink still counts (intent signal)")
    func hasAnyCachedHFRepoBrokenSymlinkCounts() throws {
        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("rapid-quickstart-cache-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tmp, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tmp) }
        // A broken symlink at ``models--<owner>--<name>`` (HF cache
        // entry whose blob target is gone) is still a strong "user
        // tried to cache something" signal — don't punish them with
        // Quickstart just because the underlying blob got GC'd.
        // ``contentsOfDirectory`` lists symlinks by name regardless
        // of target validity, so the name-prefix check captures it.
        let link = tmp.appendingPathComponent("models--mlx-community--Qwen3-0.6B-4bit")
        try FileManager.default.createSymbolicLink(
            at: link,
            withDestinationURL: URL(fileURLWithPath: "/tmp/does-not-exist-\(UUID().uuidString)")
        )
        #expect(ModelCatalog.hasAnyCachedHFRepo(hubCacheRoot: tmp) == true)
    }

    @Test("#298: hasAnyCachedHFRepo — multiple repos still return true (heavy user)")
    func hasAnyCachedHFRepoMultipleRepos() throws {
        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("rapid-quickstart-cache-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tmp, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tmp) }
        for repo in [
            ...
            try FileManager.default.createDirectory(
                at: tmp.appendingPathComponent(repo, isDirectory: true),
                withIntermediateDirectories: true
            )
        }
        #expect(ModelCatalog.hasAnyCachedHFRepo(hubCacheRoot: tmp) == true)
