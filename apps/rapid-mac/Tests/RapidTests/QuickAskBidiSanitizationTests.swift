...
struct QuickAskBidiSanitizationTests {
    ...
    /// The Quick-Ask transcript row must wrap ``msg.content`` in
    /// ``ChatTextSanitizer.sanitizeForDisplay(...)`` before passing
    /// it to the ``Markdown`` view. The pattern is intentionally
    /// strict — any whitespace-tolerant variant should match (Swift
    /// formatter rewrites are fine) but a bare
    /// ``Markdown(msg.content)`` call should fail.
    ///
    /// Codex r1 MAJOR: scope this check to the body of
    /// ``messageRow`` only. A refactor that drops the sanitiser
    /// from the render call but leaves a stale reference elsewhere
    /// in the file would otherwise pass.
    @Test("QuickAskView.messageRow sanitises assistant content before rendering")
    func quickAskMessageRowSanitisesContent() throws {
        let source = try loadSource("Sources/Rapid/QuickAsk/QuickAskView.swift")
        let slice = try messageRowSlice(source)
        #expect(
            slice.contains("ChatTextSanitizer.sanitizeForDisplay(msg.content)"),
            """
            QuickAskView.messageRow must call \
            ChatTextSanitizer.sanitizeForDisplay(msg.content) before \
            handing the value to Markdown(...). Without it, hostile \
            assistant output (or any tool-result echoed into content) \
            can ship unicode bidi-control codepoints — \
            U+202E RTL override, U+2066-9 isolates, U+200E/F marks — \
            and the chat bubble will reorder text the user sees vs \
            text they think they pasted. See bug_report.md cycle-2 \
            P3 desktop and ChatTextSanitizerTests for the codepoint set.
            """
        )
    }

    /// Defence-in-depth: pin that no ``Markdown(`` call inside
    /// ``messageRow`` passes a raw value (or any local binding /
    /// alias) without the sanitiser wrap. Closes codex r1 MAJOR
    /// (test scope) and codex r2 MAJOR (inverted-ternary leak):
    ///
    ///   1. Extract every argument substring between
    ///      ``Markdown(`` and the matching close paren.
    ///   2. Require every extracted argument contains
    ///      ``ChatTextSanitizer.sanitizeForDisplay`` — catches the
    ///      bare ``Markdown(msg.content)`` shape.
    ///   3. Require every occurrence of ``msg.content`` inside the
    ///      argument is *immediately* preceded by the sanitiser
    ///      call ``ChatTextSanitizer.sanitizeForDisplay(``. This
    ///      catches the codex r2 inverted-ternary regression:
    ///
    ///        Markdown(
    ///            msg.content.isEmpty
    ///                ? ChatTextSanitizer.sanitizeForDisplay(msg.content)
    ///                : msg.content
    ///        )
    ///
    ///      The first ``msg.content`` is bare (the ternary
    ///      condition reads the raw value — fine, it's just a
    ///      length check, no rendering involved); the second
    ///      ``msg.content`` is preceded by ``sanitizeForDisplay(``
    ///      — sanitised. The third ``msg.content`` is bare and
    ///      reaches MarkdownUI — leak. Step 3 detects the third.
    ///
    /// The ``.isEmpty`` access of the condition branch is allowed
    /// because it doesn't render anything — it's a length probe.
    /// The check is "the raw value must not flow into a rendering
    /// expression"; we approximate that with "every value use of
    /// ``msg.content`` (i.e., not followed by a property access
    /// like ``.isEmpty`` or ``.count``) must follow the sanitiser
    /// call". A use that drops into ``.isEmpty`` / ``.count`` /
    /// ``.utf8`` etc. is a probe, not a render.
    @Test("Every Markdown(...) call inside QuickAskView.messageRow routes through the sanitiser")
    func everyMessageRowMarkdownCallIsSanitised() throws {
        let source = try loadSource("Sources/Rapid/QuickAsk/QuickAskView.swift")
        let slice = try messageRowSlice(source)
        let calls = Self.extractMarkdownCallArguments(from: slice)
        #expect(
            !calls.isEmpty,
            "messageRow has no Markdown(...) calls — has the render path moved? Update this test."
        )
        for (idx, arg) in calls.enumerated() {
            #expect(
                arg.contains("ChatTextSanitizer.sanitizeForDisplay"),
                """
                Markdown(...) call #\(idx) inside messageRow does not \
                route its argument through \
                ChatTextSanitizer.sanitizeForDisplay. Argument was: \
                '\(arg)'. Any expression handed to Markdown(...) must \
                pass through the sanitiser so bidi controls (U+202E \
                etc.) can't reorder rendered text. See bug_report.md \
                cycle-2 P3 desktop.
                """
            )
            // Codex r4 MAJOR + r5 MAJOR-1: whitespace and comments
            // around member dots are valid Swift trivia. All of
            // these typecheck identically to ``msg.content``:
            //
            //   msg.content
            //   msg .content
            //   msg . content
            //   msg /*x*/.content
            //   msg . /*x*/ content
            //   msg // trailing line comment
            //     .content
            //
            // Pre-process the extracted argument by (a) stripping
            // ``/*...*/`` block comments and ``// ...`` line
            // comments, then (b) stripping every whitespace scalar.
            // The compacted form is what the subsequent substring
            // checks operate on; both the whitelist probes
            // (``.isEmpty`` etc.) and the safe-wrap prefix
            // (``ChatTextSanitizer.sanitizeForDisplay(``) contain
            // no whitespace and no comment markers themselves, so
            // compaction is order-preserving for the patterns we
            // care about.
            //
            // Known residual gap (codex r5 MAJOR-2): this test does
            // not catch local-alias leaks of the form
            //
            //   let leaked = msg.content
            //   Markdown(
            //       msg.content.isEmpty
            //           ? ChatTextSanitizer.sanitizeForDisplay(msg.content)
            //           : leaked
            //   )
            //
            // because the unsafe Markdown argument no longer
            // contains the literal ``msg.content`` token. A
            // tokenising / SwiftSyntax-based pin could trace the
            // binding, but adding that dep is out of scope for
            // this fix — it's a CI infra change, not a chat-
            // surface change. The remaining risk is "an attacker /
            // refactorer who deliberately rewrites the render
            // call to introduce an alias" which is the same shape
            // as "a human who deliberately bypasses lint". The
            // realistic accidental-regression risk (formatter
            // rewrites, swap-the-branches edits, "let me extract
            // a constant") is covered.
            let stripped = Self.stripCommentsAndWhitespace(arg)
            let compactArg = stripped

            // Codex r2/r3 MAJOR: walk every occurrence of ``msg.content``
            // in the (whitespace-stripped) argument. For each occurrence:
            //
            //   - If it's immediately followed by ``.<probe>`` where
            //     ``<probe>`` is one of a *closed* whitelist of
            //     length / identity probes that don't yield renderable
            //     content (``isEmpty``, ``count``, ``utf8.count``,
            //     ``unicodeScalars.count``, ``hashValue``,
            //     ``startIndex``, ``endIndex``), it's a probe —
            //     allowed.
            //
            //   - Otherwise the value flows into the surrounding
            //     expression and must be preceded by
            //     ``ChatTextSanitizer.sanitizeForDisplay(``.
            //
            // Codex r3 surfaced that an open exemption on "any
            // property access" lets ``msg.content.description``,
            // ``msg.content.lowercased()``, etc. through — those
            // return raw String content and re-leak the bidi
            // controls. A whitelist (rather than a member-name
            // denylist) fails closed: any new accessor a future
            // Swift / String API adds must be explicitly added here
            // before it can be used at this render site.
            let raw = "msg.content"
            ...
            while let occ = compactArg.range(of: raw, range: search..<compactArg.endIndex) {
                let after = compactArg[occ.upperBound...]
                let isProbe = probeWhitelist.contains { probe in
                    // Probe matches if the substring starts with one
                    // of the whitelist entries AND is followed by a
                    // non-identifier character (so e.g. ``.count`` is
                    // a probe but ``.counterpart`` is not).
                    ...
                    // An identifier-continuation character means
                    // the match is a prefix of a longer identifier —
                    // not a probe.
                ...
                if !isProbe {
                    // Value use — must be preceded by the safe wrap.
                    ...
                    #expect(
                        preceding == safeWrap,
                        """
                        Markdown(...) call #\(idx) inside messageRow \
                        contains a raw ``msg.content`` value use that \
                        is NOT immediately preceded by \
                        ChatTextSanitizer.sanitizeForDisplay( and is \
                        NOT a whitelisted length/identity probe \
                        (.isEmpty, .count, .utf8.count, etc.). \
                        Argument (whitespace-collapsed) was: \
                        '\(compactArg)'. This is the codex r2/r3/r4 \
                        bypass shape — the raw value reaches MarkdownUI \
                        without the bidi-control sanitiser. See \
                        bug_report.md cycle-2 P3 desktop.
                        """
                    )
                }
