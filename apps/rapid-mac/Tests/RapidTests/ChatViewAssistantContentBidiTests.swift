...
struct ChatViewAssistantContentBidiTests {
    ...
    /// The streaming hot path renders assistant content via
    /// ``Text(memoisedSanitisedContent)`` where
    /// ``memoisedSanitisedContent`` is a computed property that
    /// delegates to ``streamingContentMemo.sanitised(message.content)``.
    /// The memo is itself a thin wrapper around
    /// ``ChatTextSanitizer.sanitize`` (see ``ChatTextSanitizer.Memo``
    /// in ``ChatView.swift``).
    ///
    /// This test pins the WHOLE chain: if a future refactor splits
    /// the memo's ``sanitised(...)`` method off of
    /// ``ChatTextSanitizer.sanitize`` (e.g. inlines a more permissive
    /// per-scalar filter), or if the computed property stops calling
    /// the memo, the test goes red with a directly actionable error.
    ///
    /// The shape we pin:
    ///
    /// ```swift
    /// private var memoisedSanitisedContent: String {
    ///     streamingContentMemo.sanitised(message.content)
    /// }
    /// ```
    ///
    /// — exact whitespace-stripped form. A formatter rewrite that
    /// adds newlines / spaces is fine.
    @Test("memoisedSanitisedContent delegates to streamingContentMemo.sanitised(message.content)")
    func streamingContentMemoComputedPropertyShape() throws {
        let source = try loadSource("Sources/Rapid/UI/ChatView.swift")
        let signature = "private var memoisedSanitisedContent: String {"
        let start = try #require(
            source.range(of: signature),
            "memoisedSanitisedContent computed property not found — has it been renamed?"
        )
        let rest = source[start.upperBound...]
        let closeBrace = try #require(
            rest.range(of: "}"),
            "memoisedSanitisedContent body close-brace not found"
        )
        let body = String(rest[..<closeBrace.lowerBound])
        let compact = Self.stripCommentsAndWhitespace(body)
        #expect(
            compact == "streamingContentMemo.sanitised(message.content)",
            """
            memoisedSanitisedContent must delegate to \
            streamingContentMemo.sanitised(message.content) — and to \
            nothing else. The streaming chat bubble's Text(...) \
            renders this value verbatim, so a refactor that drops the \
            memo wrapper (or routes message.content through a \
            different / weaker filter) silently re-opens the bidi- \
            control filename-spoofing hole flagged by cycle-10 F-10-4. \
            Compacted body was: '\(compact)'. See \
            ChatTextSanitizerTests for the codepoint contract.
            """
        )
    }

    /// Sibling pin for the reasoning lane. A reasoning-only payload
    /// (Qwen / thinking models emit ``reasoning_content`` separately
    /// from ``content``) ALSO renders inside an assistant bubble via
    /// the reasoning disclosure — and a tool-result echoed into
    /// ``reasoning_content`` would carry the same bidi-control payload.
    @Test("memoisedSanitisedReasoning delegates to streamingReasoningMemo.sanitised(message.reasoning)")
    func streamingReasoningMemoComputedPropertyShape() throws {
        let source = try loadSource("Sources/Rapid/UI/ChatView.swift")
        let signature = "private var memoisedSanitisedReasoning: String {"
        let start = try #require(
            source.range(of: signature),
            "memoisedSanitisedReasoning computed property not found — has it been renamed?"
        )
        let rest = source[start.upperBound...]
        let closeBrace = try #require(
            rest.range(of: "}"),
            "memoisedSanitisedReasoning body close-brace not found"
        )
        let body = String(rest[..<closeBrace.lowerBound])
        let compact = Self.stripCommentsAndWhitespace(body)
        #expect(
            compact == "streamingReasoningMemo.sanitised(message.reasoning)",
            """
            memoisedSanitisedReasoning must delegate to \
            streamingReasoningMemo.sanitised(message.reasoning). Same \
            rationale as memoisedSanitisedContent: a Qwen / GPT-OSS \
            reasoning trace that echoes a tool result containing \
            U+202E etc. would otherwise reorder the disclosure body. \
            Compacted body was: '\(compact)'.
            """
        )
    }

    /// Belt-and-braces: the streaming render call site itself must
    /// pass ``memoisedSanitisedContent`` (NOT ``message.content``)
    /// to ``Text(...)``. Even with the computed property correctly
    /// wired, a refactor that "inlines" the property back to
    /// ``Text(message.content)`` for "perf" would re-open the leak.
    /// This test pins the call site form.
    @Test("Streaming assistant Text(...) call uses memoisedSanitisedContent, not message.content")
    func streamingAssistantTextCallSitePinned() throws {
        let source = try loadSource("Sources/Rapid/UI/ChatView.swift")
        let block = try Self.assistantBlockSlice(source)
        // The streaming branch lives inside ``if message.status == .streaming {``.
        // We grep for both possible bad shapes:
        let forbiddenShapes = [
            "Text(message.content)",
            "Text( message.content )",
        ]
        let compact = Self.stripCommentsAndWhitespace(block)
        for shape in forbiddenShapes {
            let compactShape = Self.stripCommentsAndWhitespace(shape)
            // Codex PR-#329 round 4 MAJOR: ``#expect`` inside a
            // loop body can be silently dropped by swift-testing
            // 0.99 (verified by direct DIAG print probe in round
            // 3). All looped assertions in this file use
            // ``Issue.record(...)`` instead to guarantee CI surfaces
            // every regression.
            if compact.contains(compactShape) {
                Issue.record(
                    """
                    ChatView.assistantBlock contains a raw \
                    Text(message.content) shape that bypasses \
                    memoisedSanitisedContent. Even one such call site \
                    re-opens the cycle-10 F-10-4 bidi-control echo hole. \
                    Use Text(memoisedSanitisedContent) for the streaming \
                    branch or LaTeXMarkdownView(content: \
                    ChatTextSanitizer.sanitizeForDisplay(message.content)) \
                    for the finalised branch.
                    """
                )
            }
        }
        // Positive shape: the streaming branch renders
        // Text(formattedStreamingContent) — the live-inline-markdown
        // wrapper added in the 2026-07 typography sweep. The sanitise
        // guarantee is preserved by a SECOND pin below on the
        // property's own definition, which fixes its body to
        // inlineFormatMemo.formatted(memoisedSanitisedContent):
        // sanitize THEN format, in that order, always.
        #expect(
            compact.contains("Text(formattedStreamingContent)"),
            """
            ChatView.assistantBlock no longer contains a \
            Text(formattedStreamingContent) call. The streaming hot \
            path may have been rewritten — verify it still routes \
            through ChatStreamInlineFormatter.Memo over \
            memoisedSanitisedContent (sanitize before format).
            """
        )
        // The wrapper itself must be the sanitised string fed to the
        // formatter — this is what keeps the F-10-4 guarantee provable
        // now that a formatting layer sits between the sanitiser and
        // the Text. A refactor that feeds RAW message.content to the
        // formatter re-opens the hole even with the Text pin green.
        let compactSource = Self.stripCommentsAndWhitespace(source)
        #expect(
            compactSource.contains(
                Self.stripCommentsAndWhitespace(
                    "private var formattedStreamingContent: AttributedString { inlineFormatMemo.formatted(memoisedSanitisedContent) }"
                )
            ),
            """
            formattedStreamingContent's body changed. Its input must \
            remain memoisedSanitisedContent (the sanitised string) — \
            feeding raw message.content to the inline formatter \
            re-opens the cycle-10 F-10-4 bidi-control echo hole.
            """
        )
        #expect(
            !compactSource.contains(
                Self.stripCommentsAndWhitespace("inlineFormatMemo.formatted(message.content)")
            ),
            "The inline formatter must never receive raw message.content."
        )
        // Codex PR-#329 round 1 MAJOR-1: alias rebinding catch.
        // Even with the literal ``Text(message.content)`` shape
        // forbidden above, a refactor like
        //
        //     let leaked = message.content
        //     Text(leaked)
        //
        // bypasses the literal-shape check because the unsafe
        // ``Text(...)`` argument no longer carries the
        // ``message.content`` token. Reject any
        // ``let <ident> = message.content`` binding inside
        // ``assistantBlock`` so the regression fails at the
        // binding site, not at the render site.
        Self.assertNoLocalAliasRebinding(
            inCompactArg: compact,
            rawTokens: ["message.content"],
            callShape: "ChatView.assistantBlock (streaming branch)"
        )
    }

    // MARK: - Complete-path render site

    /// The finalised assistant content renders via
    /// ``LaTeXMarkdownView(content: ChatTextSanitizer.sanitizeForDisplay(message.content))``.
    /// This is the call site cycle-10 F-10-4 said "round-trips into
    /// assistant content verbatim" if the wrap is dropped. Pin it.
    ///
    /// The check matches both the bare shape and any whitespace /
    /// comment trivia inside the argument list.
    @Test("ChatView.assistantBlock LaTeXMarkdownView(content:) wraps message.content via ChatTextSanitizer")
    func completeAssistantLaTeXMarkdownViewWrappedBySanitizer() throws {
        let source = try loadSource("Sources/Rapid/UI/ChatView.swift")
        let block = try Self.assistantBlockSlice(source)
        let calls = Self.extractMatchedCallArguments(callPrefix: "LaTeXMarkdownView(", from: block)
        #expect(
            !calls.isEmpty,
            """
            ChatView.assistantBlock has no LaTeXMarkdownView(...) \
            calls. The finalised render path may have been rewritten; \
            verify the new shape still routes message.content through \
            ChatTextSanitizer.sanitizeForDisplay before handing it to \
            MarkdownUI / SwiftUI Text.
            """
        )
        for (idx, arg) in calls.enumerated() {
            let compactArg = Self.stripCommentsAndWhitespace(arg)
            // Two requirements (matches the QuickAsk pattern):
            //   1. the sanitiser is mentioned at all (catches the
            //      bare LaTeXMarkdownView(content: message.content) shape);
            //   2. every value-use of message.content is immediately
            //      preceded by ChatTextSanitizer.sanitizeForDisplay(
            //      OR is a whitelisted length/identity probe.
            // Codex PR-#329 round 4 MAJOR: loop-body ``#expect`` ->
            // ``Issue.record`` so swift-testing 0.99 cannot drop
            // the failure silently.
            if !compactArg.contains("ChatTextSanitizer.sanitizeForDisplay") {
                Issue.record(
                    """
                    LaTeXMarkdownView(...) call #\(idx) inside \
                    assistantBlock does not route its argument through \
                    ChatTextSanitizer.sanitizeForDisplay. Compact arg: \
                    '\(compactArg)'.
                    """
                )
            }
            Self.assertEveryMessageContentUseIsSanitised(
                inCompactArg: compactArg,
                callIdx: idx,
                callShape: "LaTeXMarkdownView"
            )
        }
        // Codex PR-#329 round 1 MAJOR-2: alias rebinding catch.
        // A refactor like
        //
        //     let leaked = message.content
        //     LaTeXMarkdownView(
        ...
        //     )
        //
        // passes the per-occurrence walker (the inverted-ternary
        // safe branch contains the sanitiser wrap; the unsafe
        // branch carries ``leaked``, not ``message.content``) but
        // still leaks raw content into MarkdownUI. Reject any
        // ``let <ident> = message.content`` binding inside the
        // ``assistantBlock`` slice so the regression goes red at
        // the binding site.
        let compactBlock = Self.stripCommentsAndWhitespace(block)
        Self.assertNoLocalAliasRebinding(
            inCompactArg: compactBlock,
            rawTokens: ["message.content"],
            callShape: "ChatView.assistantBlock (complete branch)"
        )
    }

    // MARK: - Tool-result render site (cycle-2 cross-confirm)

    /// A malicious tool result (the cycle-2 finding referenced
    /// "tool-result spoofing") could echo bidi controls back into the
    /// ``ToolCallChip`` result body. Pin that the result body Text(...)
    /// call inside ``ToolCallChip.body`` routes through the sanitiser.
    ///
    /// The render call today is:
    ///
    /// ```swift
    /// Text(r.content.isEmpty
    ///      ? "(empty result)"
    ///      : ChatTextSanitizer.sanitizeForDisplay(r.content))
    /// ```
    ///
    /// — ternary with the safe wrap in the else branch and a literal
    /// fallback in the then branch. Pin both halves so a refactor
    /// that inverts the ternary (or drops the wrap) goes red.
    @Test("ToolCallChip result-body Text(...) wraps r.content via ChatTextSanitizer")
    func toolResultBodyTextWrappedBySanitizer() throws {
        let source = try loadSource("Sources/Rapid/UI/ChatView.swift")
        let chip = try Self.toolCallChipBodySlice(source)
        let compact = Self.stripCommentsAndWhitespace(chip)

        // Codex PR-#329 round 1 MAJOR-4: positive assertion that
        // the sanitiser is mentioned at all somewhere inside the
        // struct slice. Without this, a refactor like
        // ``Text(result?.content ?? "")`` drops both the literal
        // ``r.content`` token AND the safe-wrap; the per-occurrence
        // walker below (which only fires on ``r.content`` matches)
        // would silently no-op.
        #expect(
            compact.contains("ChatTextSanitizer.sanitizeForDisplay"),
            """
            ToolCallChip body no longer mentions \
            ChatTextSanitizer.sanitizeForDisplay anywhere — the \
            tool-result render site has been refactored in a way that \
            doesn't route through the bidi-control sanitiser. \
            A common shape this catches: Text(result?.content ?? "") \
            (drops the r.content alias AND the sanitiser wrap). See \
            bug_report.md cycle-10 F-10-4.
            """
        )

        // Walk every ``r.content`` value-use the same way the
        // QuickAsk test walks ``msg.content`` — each must either be
        // a whitelisted probe (``.isEmpty`` etc.) or be preceded by
        // ``ChatTextSanitizer.sanitizeForDisplay(``.
        Self.assertEveryRContentUseIsSanitised(
            inCompactArg: compact,
            callShape: "ToolCallChip.body"
        )

        // Codex PR-#329 round 1 MAJOR-4 (sibling): also walk every
        // ``result?.content`` value-use (without the ``r = result``
        // binding) for the same safe-wrap-or-probe contract. The
        // optional-chain shape is a likely refactor target if the
        // ``if let r = result {...}`` binding ever gets removed for
        // brevity.
        Self.assertEveryRawValueUseIsSanitised(
            inCompactArg: compact,
            rawToken: "result?.content",
            callIdx: 0,
            callShape: "ToolCallChip.body (result?.content)"
        )

        // Codex PR-#329 round 1 MAJOR-1/2: alias rebinding catch.
        // A regression that does ``let leaked = r.content; Text(leaked)``
        // — or the same shape with ``result?.content`` or
        // ``message.content`` (see assistantBlock siblings) —
        // bypasses the per-occurrence walker because the unsafe
        // Markdown / Text argument no longer carries the literal
        // raw-value token. Reject any ``let <ident> = <rawToken>``
        // shape inside the slice so the regression is caught at the
        // binding site, not at the render site.
        Self.assertNoLocalAliasRebinding(
            inCompactArg: compact,
            rawTokens: ["r.content", "result?.content"],
            callShape: "ToolCallChip.body"
        )
    }
